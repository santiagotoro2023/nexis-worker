...
class NexisApiService {
    ...
    suspend fun abortChat(baseUrl: String, token: String) = runCatching {
        client.post("$baseUrl/api/chat/abort") {
            contentType(ContentType.Application.Json)
            header("Authorization", "Bearer $token")
            setBody("{}")
        }
    ...
    suspend fun getModels(baseUrl: String, token: String): List<ModelInfo> = runCatching {
        ...
        arr.map { el ->
            val o = el.jsonObject
            ModelInfo(o.str("key"), o.str("label"), o.str("desc"),
                      o["installed"]?.jsonPrimitive?.boolean ?: true,
                      o["current"]?.jsonPrimitive?.boolean ?: false)
        }
    }.getOrDefault(emptyList())

    suspend fun setModel(baseUrl: String, token: String, modelKey: String) = runCatching {
        client.post("$baseUrl/api/model") {
            contentType(ContentType.Application.Json)
            header("Authorization", "Bearer $token")
            setBody("""{"model":"${modelKey.esc}"}""")
        }
    }
    ...
    data class HealthInfo(val model: String, val modelLabel: String, val voice: Boolean,
                          val voiceModel: String, val memories: Int, val histLen: Int,
    ...
    suspend fun getHealth(baseUrl: String, token: String): HealthInfo? = runCatching {
        ...
        HealthInfo(o.str("model"), o.str("model_label"),
                   o["voice"]?.jsonPrimitive?.boolean ?: false, o.str("voice_model"),
                   o["memories"]?.jsonPrimitive?.int ?: 0,
                   o["hist_len"]?.jsonPrimitive?.int ?: 0,
                   o["uptime"]?.jsonPrimitive?.int ?: 0)
    }.getOrNull()
    ...
    data class SessionSummary(val sessionId: String, val started: String,
                              val title: String, val preview: List<SessionMessage>)
    ...
    suspend fun getHistorySessions(baseUrl: String, token: String): List<SessionSummary> = runCatching {
        ...
        arr.map { el ->
            ...
            SessionSummary(
                ...
                started   = o.str("started"),
                title     = o["title"]?.jsonPrimitive?.content ?: "",
                preview   = prev.map { m ->
                    val mo = m.jsonObject
                    SessionMessage(mo.str("role"), mo.str("content"))
                },
            )
        }
    }.getOrDefault(emptyList())
    ...
    suspend fun deleteHistorySession(baseUrl: String, token: String, sessionId: String) = runCatching {
        client.post("$baseUrl/api/history/sessions") {
            contentType(ContentType.Application.Json)
            header("Authorization", "Bearer $token")
            setBody("""{"action":"delete","session_id":"${sessionId.esc}"}""")
        }
    }

    suspend fun loadHistorySession(baseUrl: String, token: String, sessionId: String) = runCatching {
        client.post("$baseUrl/api/history/load") {
            contentType(ContentType.Application.Json)
            header("Authorization", "Bearer $token")
            setBody("""{"session_id":"${sessionId.esc}"}""")
        }
    }

    suspend fun clearConversation(baseUrl: String, token: String) = runCatching {
        client.post("$baseUrl/api/clear") {
            contentType(ContentType.Application.Json)
            header("Authorization", "Bearer $token")
            setBody("{}")
        }
    ...
    suspend fun getMemories(baseUrl: String, token: String): List<MemoryEntry> = runCatching {
        ...
        arr.map { el ->
            val o = el.jsonObject
            MemoryEntry(o["id"]?.jsonPrimitive?.int ?: 0, o.str("content"),
                        o["created_at"]?.jsonPrimitive?.content ?: "")
        }
    }.getOrDefault(emptyList())

    suspend fun deleteMemory(baseUrl: String, token: String, id: Int) = runCatching {
        client.post("$baseUrl/api/memories") {
            contentType(ContentType.Application.Json)
            header("Authorization", "Bearer $token")
            setBody("""{"action":"delete","id":$id}""")
        }
    }
    ...
    suspend fun getSchedules(baseUrl: String, token: String): List<ScheduleEntry> = runCatching {
        ...
        arr.map { el ->
            val o = el.jsonObject
            ScheduleEntry(o["id"]?.jsonPrimitive?.int ?: 0, o.str("name"), o.str("expr"),
                          o.str("prompt"), o["active"]?.jsonPrimitive?.boolean ?: true,
                          o["last_run"]?.jsonPrimitive?.contentOrNull)
        }
    }.getOrDefault(emptyList())

    suspend fun scheduleAction(baseUrl: String, token: String, action: String,
                               id: Int? = null, name: String? = null,
                               expr: String? = null, prompt: String? = null,
                               active: Boolean? = null) = runCatching {
        val obj = buildJsonObject {
            put("action", action)
            if (id     != null) put("id", id)
            if (name   != null) put("name", name)
            if (expr   != null) put("expr", expr)
    ...
    suspend fun getCommands(baseUrl: String, token: String): List<CommandEntry> = runCatching {
        ...
        arr.map { el ->
            val o = el.jsonObject
            CommandEntry(o["id"]?.jsonPrimitive?.int ?: 0, o.str("label"), o.str("cmd"), o.str("icon"))
        }
    }.getOrDefault(emptyList())

    suspend fun runCommand(baseUrl: String, token: String, cmd: String): String = runCatching {
        val resp = client.post("$baseUrl/api/command") {
            contentType(ContentType.Application.Json)
            header("Authorization", "Bearer $token")
            setBody("""{"cmd":"${cmd.esc}"}""")
        }
    ...
    suspend fun getPersonality(baseUrl: String, token: String): PersonalityInfo? = runCatching {
        ...
        if (!resp.status.isSuccess()) return null
        val o = Json.parseToJsonElement(resp.bodyAsText()).jsonObject
        PersonalityInfo(o.str("name"), o.str("system_prompt"),
                        o["voice_enabled"]?.jsonPrimitive?.boolean ?: false)
    }.getOrNull()

    suspend fun savePersonality(baseUrl: String, token: String, name: String,
                                systemPrompt: String, voiceEnabled: Boolean) = runCatching {
        client.post("$baseUrl/api/personality") {
            contentType(ContentType.Application.Json)
            header("Authorization", "Bearer $token")
            setBody("""{"name":"${name.esc}","system_prompt":"${systemPrompt.esc}","voice_enabled":$voiceEnabled}""")
        }
    }

    // ── Devices ───────────────────────────────────────────────────────────────

    data class DeviceInfo(val deviceId: String, val hostname: String, val os: String,
                          val deviceType: String, val online: Boolean,
                          val batteryPct: Int?, val charging: Boolean?,
                          val ip: String, val mac: String, val role: String?,
                          val lastSeen: String)
    ...
    suspend fun getDevices(baseUrl: String, token: String): List<DeviceInfo> = runCatching {
        ...
        arr.map { el ->
            ...
            DeviceInfo(
                ...
                os         = o.str("os"),
                deviceType = o["device_type"]?.jsonPrimitive?.content ?: "desktop",
                online     = o["online"]?.jsonPrimitive?.boolean ?: false,
                batteryPct = o["battery_pct"]?.jsonPrimitive?.intOrNull,
                charging   = o["charging"]?.jsonPrimitive?.booleanOrNull,
                ip         = o.str("ip"),
                mac        = o.str("mac"),
                role       = o["role"]?.jsonPrimitive?.contentOrNull?.takeIf { it.isNotEmpty() && it != "null" },
                lastSeen   = o.str("last_seen"),
            )
        }
    }.getOrDefault(emptyList())

    suspend fun sendDeviceCommand(baseUrl: String, token: String,
                                   deviceId: String, action: String, arg: String = ""): String = runCatching {
        val resp = client.post("$baseUrl/api/device/command") {
            contentType(ContentType.Application.Json)
            header("Authorization", "Bearer $token")
            setBody("""{"device_id":"${deviceId.esc}","action":"${action.esc}","arg":"${arg.esc}"}""")
        }
        val o = Json.parseToJsonElement(resp.bodyAsText()).jsonObject
    ...
    suspend fun wakeOnLan(baseUrl: String, token: String, mac: String): String = runCatching {
        val resp = client.post("$baseUrl/api/wol") {
            contentType(ContentType.Application.Json)
            header("Authorization", "Bearer $token")
            setBody("""{"mac":"${mac.esc}"}""")
        }
    ...
    data class HvMetrics(val cpu: Double, val mem: Double, val disk: Double,
                         val vmsTotal: Int, val vmsActive: Int, val ctsTotal: Int, val ctsActive: Int)

    suspend fun getHvToken(hvUrl: String, username: String, password: String): String {
        val resp = client.post("$hvUrl/api/auth/login") {
            contentType(ContentType.Application.Json)
            setBody("""{"username":"${username.esc}","password":"${password.esc}"}""")
        }
    ...
    suspend fun listHvVms(hvUrl: String, hvToken: String): List<HvVm> = runCatching {
        ...
        arr.map { el ->
            val o = el.jsonObject
            HvVm(o.str("id"), o.str("name"), o.str("status"),
                 o["vcpus"]?.jsonPrimitive?.int ?: 1, o["memory_mb"]?.jsonPrimitive?.long ?: 0)
        }
    }.getOrDefault(emptyList())

    suspend fun hvVmAction(hvUrl: String, hvToken: String, vmId: String, action: String) = runCatching {
        client.post("$hvUrl/api/vms/$vmId/$action") {
            contentType(ContentType.Application.Json)
    ...
    suspend fun listHvContainers(hvUrl: String, hvToken: String): List<HvContainer> = runCatching {
        ...
        arr.map { el ->
            val o = el.jsonObject
            HvContainer(o.str("name"), o.str("status"), o["memory_mb"]?.jsonPrimitive?.long ?: 0)
        }
    }.getOrDefault(emptyList())

    suspend fun hvContainerAction(hvUrl: String, hvToken: String, ctName: String, action: String) = runCatching {
        client.post("$hvUrl/api/containers/$ctName/$action") {
            contentType(ContentType.Application.Json)
            header("Authorization", "Bearer $hvToken")
            setBody("{}")
        }
    ...
    suspend fun getHvMetrics(hvUrl: String, hvToken: String): HvMetrics? = runCatching {
        ...
        if (!resp.status.isSuccess()) return null
        val o = Json.parseToJsonElement(resp.bodyAsText()).jsonObject
        HvMetrics(o["cpu"]?.jsonPrimitive?.double ?: 0.0, o["mem"]?.jsonPrimitive?.double ?: 0.0,
                  o["disk"]?.jsonPrimitive?.double ?: 0.0,
    ...
    suspend fun haAction(baseUrl: String, token: String, action: String) = runCatching {
        client.post("$baseUrl/api/ha/action") {
            contentType(ContentType.Application.Json)
            header("Authorization", "Bearer $token")
            setBody("""{"action":"${action.esc}"}""")
        }
    }

    // ── Remote desktop ────────────────────────────────────────────────────────

    suspend fun desktopAction(baseUrl: String, token: String, action: String,
                               arg: String = "", deviceId: String = ""): String = runCatching {
        val obj = buildJsonObject {
            put("action", action)
            put("arg", arg)
        ...
        Json.parseToJsonElement(text).jsonObject["result"]?.jsonPrimitive?.content ?: "(no result)"
    }.getOrDefault("(error)")
    ...
    suspend fun getMonitorStats(baseUrl: String, token: String): MonitorStats? = runCatching {
        ...
        if (!resp.status.isSuccess()) return null
        val o = Json.parseToJsonElement(resp.bodyAsText()).jsonObject
        MonitorStats(o["cpu"]?.jsonPrimitive?.float ?: 0f,
                     o["mem"]?.jsonPrimitive?.float ?: 0f,
                     o["disk"]?.jsonPrimitive?.float ?: 0f)
    }.getOrNull()
